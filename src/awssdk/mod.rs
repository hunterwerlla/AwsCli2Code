...
use crate::awssdk::definition_parser::{Api, Paginators1, Service2Shape, ServiceModel};
use definition_parser::Service2;
use std::collections::HashMap;
use std::fs::File;
use std::path::Path;

fn resolve(service: Service2, paginators: Option<Paginators1>) -> ServiceModel {
    ...
    for (key, value) in &service.operations {
        ...
        let struct_shape = match shapes.get(input) {
            None => panic!("Invalid top level shape {} specified!", input),
            Some(s) => match s {
                Service2Shape::Structure { required, members } => required,
...
pub fn load_and_parse_service(path: &str) -> Result<ServiceModel, std::io::Error> {
    ...
    Ok(resolve(service2, paginators))
}
